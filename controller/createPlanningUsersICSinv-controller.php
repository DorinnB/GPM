...
foreach ($period as $key => $value) {
  ...
  if (isset($planning[$value->format("Y-m-d")][$_COOKIE['id_user']])) { //entrée dans l'agenda GPM
    ...
    elseif (($planning[$value->format("Y-m-d")][$_COOKIE['id_user']]['workable']!=1 OR $planning[$value->format("Y-m-d")][$_COOKIE['id_user']]['quantity']<1) AND $value->format("N")<6) {  // non travaillé en semaine
    ...
    elseif ($planning[$value->format("Y-m-d")][$_COOKIE['id_user']]['workable']==1 AND $value->format("N")==6) {  // travail le samedi
