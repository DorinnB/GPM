...
->fields(
  ...
  ->leftJoin('ubr', 'ubr.job=info_jobs.job and ubr.date_UBR = "'.$_POST['dateStartUBR'].'"','','')
  ->leftJoin('ubr as ubrold', 'ubrold.job=info_jobs.job and ubrold.date_UBR = "'.date("Y-m-t",strtotime(date("Y-m-t", strtotime($_POST['dateStartUBR'])) . "-35 days")).'"','','')
  ...
  ->where( function ( $q ) {
    ...
    $q->or_where( function($r) {
      $r->where( 'info_jobs.datecreation',$_POST['dateStartUBR'],'<=');
      $r->where( 'info_jobs.datecreation',date("Y-m-01",strtotime($_POST['dateStartUBR'])),'>');
    });
