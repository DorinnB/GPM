...
class EprouvetteModel
{
  ...
  public function updateDCheck($iduser){
    ...
    $maReponse = array('result' => 'ok', 'req'=> $reqUpdate, 'id_eprouvette' => $this->id, 'id_user' => $iduser, 'is_Trans' => "1");

    $req='SELECT *
    FROM enregistrementessais
    WHERE id_eprouvette='.$this->id.';';
    //echo $req.'<br/>';
    $isOne = $this->db->getOne($req);

    if ($isOne) {
      $ini = parse_ini_file('../var/config.ini');
      $trans = $ini['PATH_TRANS'].$isOne['n_fichier'];
      if (!is_dir($trans)) {
        $maReponse['is_Trans']="0";
      }
    }
  ...
  public function update_fractureCust()  {

    $reqUpdate='UPDATE `eprouvettes` SET
    `fractureCust` = '.$this->fractureCust.',
    check_rupture = -'.$this->iduser.'
    WHERE `eprouvettes`.`id_eprouvette` = '.$this->id.';';

    $result = $this->db->execute($reqUpdate);

    $maReponse_Fracture = array('result' => 'fractureCust', 'req'=> $reqUpdate, 'id_eprouvette' => $this->id, 'id_user' => $this->iduser);
  ...
  public function getTest() {
    ...
    Rupture, Fracture, fractureCust, other_1, other_2, other_3, other_4, other_5,
    ...
    signal_true, signal_tapered, young, flag_qualite, check_rupture, rawdatasent, valid, unvalidity, validupto,
    CASE
    WHEN valid=1 THEN "Valid"
    WHEN valid=2 THEN "Unvalid"
    WHEN valid=3 THEN "Void"
    END as validation,
