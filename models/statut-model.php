...
class StatutModel
{
  ...
  public function removeNewCondition(){
    ...
    $statut=$this->db->getOne($req);
    if ($statut['id_statut_temp']=="53") {
      $this->updateStatut2(50);
    }
  }
  ...
  public function findStatut(){
    ...
    FROM tbljobs
    WHERE id_info_job= (SELECT t2.id_info_job FROM tbljobs t2 WHERE t2.id_tbljob='.$this->id_tbljob.');';

    foreach ($this->db->getAll($req) as $key => $value) {
      $this->id_tbljob=$value['id_tbljob'];
      $this->findSplitStatut();
    }
  }
  ...
  public function findSplitStatut(){
    $req='SELECT
    ...
    sum(
      if(
        ( master_eprouvette_inOut_A is null OR
          IFNULL(
            (SELECT if((ep.eprouvette_inOut_B is not null or ep.report_creation_date is not null or ep.d_checked >0),0,1)
            FROM eprouvettes ep
            LEFT JOIN tbljobs tbl ON tbl.id_tbljob=ep.id_job
        ...
        ) OR (eprouvette_inOut_A is not null or enregistrementessais.date is not null),
        0,
        1
      )
    ) as nb_ep_dispo,
    ...
    sum(
      ...
      )  OR (eprouvette_inOut_A is not null or enregistrementessais.date is not null)) OR (c_checked<=0 OR c_checked IS NULL),
      0,
      1)
